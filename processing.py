...
month_var = input('type filename from data_files folder without extansion: ').split()
for i in range(len(month_var)):
    df = pd.read_csv("C:/py_projects/stats_hotel/data_files/main", header=[0, 1], index_col=0)
    month = pd.read_excel(f'C:/py_projects/stats_hotel/data_files/{month_var[i]}.xlsx', index_col='Показатель')
    ...
    month = month.rename(columns={'Значение за период': f'{month_var[i]}'})
    ...
    os.remove(f'C:/py_projects/stats_hotel/data_files/{month_var[i]}.xlsx')

    print(f'main is extended by {month_var[i]}')
